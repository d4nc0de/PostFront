...
export class AppConfigurator {
    ...
    ngOnInit() {
        if (isPlatformBrowser(this.platformId)) {
            // 1) Preset actual (no lo cambies si ya lo traes de tu LayoutService)
            const presetName = this.layoutService.layoutConfig().preset ?? 'Aura';
            this.layoutService.layoutConfig.update((s) => ({ ...s, preset: presetName }));

            // 2) Defaults que quieres
            ...
            // 3) Aplica tema (preset + primary rose + surface slate)
            const presetObj = presets[presetName as keyof typeof presets];
            const surfacePalette = this.surfaces.find((s) => s.name === 'slate')?.palette;

            // Primary (rose) sale de getPresetExt() porque mira selectedPrimaryColor()
            updatePreset(this.getPresetExt());

            // Surface (slate)
            if (surfacePalette) updateSurfacePalette(surfacePalette);

            // Amarra todo al theme engine
            $t()
            .preset(presetObj)
            ...
            // (Opcional) si quieres además disparar la lógica que ya tienes:
